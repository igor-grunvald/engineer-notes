...
**На уровне кода** метод `AbstractAutowireCapableBeanFactory.doCreateBean()` проходит следующие шаги:
...
**Разрушение** при закрытии контекста:
...
**Важные промежуточные шаги:**
- **`MergedBeanDefinitionPostProcessor`** (шаг 2): сканирует бин на `@Autowired`, `@Value`, `@PostConstruct` и сохраняет метаданные в `InjectionMetadata`. Включает `AutowiredAnnotationBeanPostProcessor`, `CommonAnnotationBeanPostProcessor`.
- **`SmartInstantiationAwareBeanPostProcessor`**: вызывается при разрешении циклических зависимостей — метод `getEarlyBeanReference()` позволяет вернуть ранний прокси из L3-кэша.
- **`DestructionAwareBeanPostProcessor`**: требуется для кастомной логики при destroy (например, освобождение ресурсов пула соединений).
...
**`DefaultListableBeanFactory`** — центральная реализация IoC-контейнера Spring. Реализует все ключевые интерфейсы: `BeanDefinitionRegistry`, `ListableBeanFactory`, `SingletonBeanRegistry`, `ConfigurableBeanFactory`.
...
**Алгоритм `doGetBean()` (упрощённо):**

```java
protected <T> T doGetBean(String name, Class<T> requiredType, Object[] args) {
    // 1. Извлечь имя бина (убрать & для FactoryBean)
    String beanName = transformedBeanName(name);

    // 2. Проверить singletonObjects — если есть готовый, вернуть
    Object sharedInstance = getSingleton(beanName);
    if (sharedInstance != null) return (T) getObjectForBeanInstance(sharedInstance, name, beanName);

    // 3. Проверить, не создаётся ли бин прямо сейчас (циклическая зависимость)
    if (isPrototypeCurrentlyInCreation(beanName)) throw new BeanCurrentlyInCreationException(beanName);

    // 4. Найти BeanDefinition (родительский контекст если нет)
    BeanDefinition bd = getMergedLocalBeanDefinition(beanName);

    // 5. Зависимости (depends-on) — создать сначала их
    String[] dependsOn = bd.getDependsOn();
    if (dependsOn != null) {
        for (String dep : dependsOn) registerDependentBean(dep, beanName), getBean(dep);
    }
...
**Ключевые моменты:**
- `getSingleton(String, ObjectFactory)` — лямбда-фабрика регистрируется в L3-кэш ДО вызова `createBean()` (это критично для разрешения циклических зависимостей)
- `createBean()` → `doCreateBean()` — полный жизненный цикл: конструктор → populate → initialize
- `getObjectForBeanInstance()` — распаковывает `FactoryBean`, если имя начиналось с `&`
- При отсутствии бина: `NoSuchBeanDefinitionException`; при нескольких кандидатах: `NoUniqueBeanDefinitionException`
...
**Ключевой метод** — `DefaultSingletonBeanRegistry.getSingleton(String, ObjectFactory)`:
...
public Object getSingleton(String beanName, ObjectFactory<?> singletonFactory) {
    synchronized (this.singletonObjects) {
        ...
        if (singletonObject == null) {
            beforeSingletonCreation(beanName);  // пометить "в процессе создания"
            try {
                // ↓ фабрика регистрируется в L3-кэш ДО вызова createBean()
                singletonObject = singletonFactory.getObject();
                // ...
            } finally {
                afterSingletonCreation(beanName);  // убрать метку
            }
            if (newSingleton) {
...
**addSingletonFactory() вызывается в `doCreateBean()` между конструктором и populate:**
```java
// doCreateBean():
instance = createBeanInstance(beanName, mbd, args);    // конструктор
addSingletonFactory(beanName, () -> getEarlyBeanReference(beanName, mbd, instance));
populateBean(beanName, mbd, instance);                  // внедрение зависимостей
```

**Фабрика в L3 может вернуть ранний прокси** через `SmartInstantiationAwareBeanPostProcessor.getEarlyBeanReference()`. Это критично: если бин А должен быть проксирован (AOP), циклическая ссылка из Б должна получить уже прокси, а не сырой объект.

**Полный алгоритм (А ↔ Б):**
1. `getSingleton("A")` → `beforeSingletonCreation("A")` → `singletonFactoryA` → L3
2. `createBean("A")` → конструктор А (сырой объект `a@123`)
3. `addSingletonFactory("A", getEarlyBeanReference(a))` → L3
4. `populateBean(A)` → нужен Б
5. `getSingleton("B")` → `beforeSingletonCreation("B")` → `singletonFactoryB` → L3
6. `createBean("B")` → конструктор Б → `addSingletonFactory("B", getEarlyBeanReference(b))` → L3
7. `populateBean(B)` → нужен А
8. `getSingleton("A")` → `singletonObjects.get("A")` = null → но А помечен `inCreation` → проверяем `earlySingletonObjects` (L2) → null → `singletonFactories.get("A")` (L3) → вызывает `getEarlyBeanReference(a)` → получает прокси/сырой А → помещает в L2
9. Б завершает populate → Б готов → `addSingleton("B", b)` → L1
10. А получает готовый Б, завершает populate → А готов → `addSingleton("A", a)` → L1

**Почему конструкторная инъекция не работает:** `addSingletonFactory()` вызывается только ПОСЛЕ конструктора. Если конструктору нужен Б, а Б — А, ни один конструктор не может завершиться → фабрики не регистрируются → разрешать нечего.

**В Spring Boot 2.6+** циклические зависимости запрещены по умолчанию (`spring.main.allow-circular-references=false`). Включить: `spring.main.allow-circular-references=true`.
