...
- **JPA (Jakarta Persistence API, ранее Java Persistence API)** — спецификация (набор интерфейсов и аннотаций) для ORM в Java. Определяет стандарт: `EntityManager`, `@Entity`, `@Table`, JPQL. Не содержит реализации.
- **Hibernate ORM** — конкретная реализация JPA (один из провайдеров). Содержит всю логику: генерация SQL, кэширование, dirty checking, проксирование. Другие провайдеры: EclipseLink, Apache OpenJPA.

Hibernate расширяет JPA дополнительными возможностями: `@Subselect`, `@Where`, `@Filter`, `@BatchSize`, `@Immutable`, многоуровневое кэширование.

### 1.2. Архитектура Hibernate

Основные компоненты:
- **SessionFactory** (JPA: `EntityManagerFactory`): тяжёлый потокобезопасный объект, создаётся один раз. Хранит метаданные маппинга, конфигурацию, фабрику L2-кэша.
- **Session** (JPA: `EntityManager`): лёгкий, однопоточный объект для одной единицы работы. Управляет Persistence Context (L1-кэш).
- **Transaction**: единица работы, в рамках которой изменения либо все коммитятся, либо все откатываются.
- **Persistence Context**: L1-кэш, хранит все управляемые сущности в рамках Session.

### 1.3. Session vs EntityManager

`Session` — Hibernate-специфичный интерфейс (пакет `org.hibernate`). `EntityManager` — JPA-стандартный интерфейс (`jakarta.persistence`). `Session` расширяет `EntityManager` дополнительными методами: `createCriteria()`, `createSQLQuery()`, `setFlushMode()`, `setCacheMode()`. В Spring Boot обычно работают через `EntityManager`, но при необходимости можно получить `Session`: `entityManager.unwrap(Session.class)`.
...
- Создаётся **один раз** при старте приложения на основе `persistence.xml` (или автоконфигурации Spring Boot)
- Содержит метаданные о всех сущностях (`EntityType`, маппинги таблиц, связей)
- Объект тяжёлый и потокобезопасный
- Создаёт `EntityManager` для каждой транзакции или запроса
- В Spring Boot конфигурируется через `LocalContainerEntityManagerFactoryBean`

### 2.2. Как работает Persistence Context (L1-кэш)?

**Persistence Context** — контекст персистентности, L1-кэш Hibernate. Содержит все управляемые (managed) сущности в рамках одной Session/EntityManager.

**Внутренние структуры данных:**

| Структура | Тип | Назначение |
|---|---|---|
| **Identity Map** | `Map<EntityUniqueKey, Object>` | Ключ = тип сущности + ID. Гарантирует: один и тот же Java-объект для одной и той же строки БД |
| **EntityEntry** | `Map<Object, EntityEntry>` | Для каждой managed-сущности: статус (MANAGED/READ_ONLY/DELETED), снимок `loadedState` (Object[]) |
| **loadedState** | `Object[]` внутри `EntityEntry` | Значения полей при загрузке из БД (или после последнего flush). Основа для dirty checking |
| **PersistentCollection** | Обёртки для LAZY-коллекций | Хранят флаг `dirty`, отслеживают добавление/удаление элементов |

**Как операции взаимодействуют с Identity Map:**

- **`persist(entity)`**: создаёт `EntityEntry(status=SAVING)`, помещает в Identity Map, планирует `INSERT` action
- **`find(Class, id)`**: проверяет Identity Map (по `EntityUniqueKey`) → если нет: L2-кэш → БД → создаёт `EntityEntry` + снимок `loadedState` → Identity Map
- **`merge(entity)`**: проверяет Identity Map по ID → если нет: загружает из БД (создаёт `EntityEntry`) → копирует поля из переданного объекта → возвращает managed-копию
- **`remove(entity)`**: `EntityEntry.status = DELETED`, планирует `DELETE` action. Объект остаётся в Identity Map до закрытия/очистки PC
- **`flush()`**: итерирует ВСЕ `EntityEntry` в PC → для каждого `MANAGED`: сравнивает текущие значения полей со снимком `loadedState` (dirty checking) → генерирует UPDATE для изменённых

**Важные свойства:**
1. **Повторяемое чтение по identity**: `find(User.class, 1L)` дважды в одной сессии → один SQL-запрос (первый раз), второй раз из Identity Map
2. **Защита от потерянных изменений в рамках одной сессии**: если объект уже в Identity Map, Hibernate всегда возвращает его, даже если БД изменилась
3. **Рост памяти**: все managed-сущности + их снимки хранятся до `clear()` или `close()`. При пакетной обработке 100K записей → 100K × (размер сущности + размер снимка) → OOM без периодического `flush()` + `clear()`

### 2.3. Как @PersistenceContext внедряет EntityManager в Spring?

`@PersistenceContext` внедряет **прокси** EntityManager (не реальный объект). Прокси (`SharedEntityManagerInvocationHandler`) для каждой операции:
1. Получает реальный EntityManager из текущей транзакции
2. Если транзакции нет — создаёт новый EntityManager на время операции
3. После завершения — возвращает EntityManager в пул/закрывает

Это позволяет использовать `@PersistenceContext` в singleton-бинах (сервисах, репозиториях) без проблем потокобезопасности.
...
| В Spring Boot | `LocalContainerEntityManagerFactoryBean` | Можно получить через `entityManagerFactory.unwrap(SessionFactory.class)` |
...
1. **Transient (новый)**: объект создан через `new`, не связан с БД и Persistence Context. Удаление такого объекта — просто dereference.
2. **Managed (управляемый)**: объект находится в Persistence Context. Все изменения автоматически синхронизируются с БД при flush. Попадает через `persist()`, `merge()`, `find()`.
3. **Detached (отсоединённый)**: объект был в Persistence Context, но сессия закрылась. Изменения не синхронизируются. Можно вернуть через `merge()`.
4. **Removed (удалённый)**: объект помечен на удаление (`remove()`). При flush генерируется DELETE.

### 3.2. persist() vs merge() vs save() vs update()

- **persist(entity)**: добавляет новый transient-объект в Persistence Context → становится managed. INSERT откладывается до flush. Если сущность уже managed — исключение `EntityExistsException`. **Возвращает void**.
- **merge(entity)**: принимает detached-объект, копирует его поля в managed-экземпляр (загружая из БД если нужно). Исходный объект остаётся detached. **Возвращает managed-копию**.
- **save(entity)** (Hibernate native): возвращает сгенерированный ID немедленно (может выполнить INSERT сразу). В JPA рекомендуется `persist()`.
- **update(entity)** (Hibernate native): переводит detached в managed. В JPA — `merge()`.

### 3.3. flush() — как и когда?

**flush()** синхронизирует Persistence Context с БД: генерирует SQL (INSERT/UPDATE/DELETE) и отправляет в БД (но не коммитит).

**Когда вызывается автоматически:**
- Перед коммитом транзакции (`commit()`)
- Перед выполнением запроса, если есть ожидающие изменения (чтобы запрос видел актуальные данные)
- При вызове `entityManager.flush()` явно

**FlushMode:**
- `AUTO` (по умолчанию) — перед коммитом и перед запросом
- `COMMIT` — только перед коммитом (производительнее, но запросы могут не видеть незакоммиченные изменения)

### 3.4. clear(), detach(), refresh(), contains()

- **clear()**: очищает весь Persistence Context → все сущности становятся detached
- **detach(entity)**: удаляет конкретную сущность из Persistence Context
- **refresh(entity)**: перезагружает состояние из БД, отменяя незакоммиченные изменения
- **contains(entity)**: проверяет, находится ли сущность в Persistence Context

### 3.5. find() vs getReference()

- **find(Class, id)**: возвращает сущность (из L1 → из L2 → из БД) или `null`
- **getReference(Class, id)**: возвращает **прокси** без загрузки данных. Если обратиться к полю прокси — загрузит. Если сущности нет — `EntityNotFoundException` при обращении к полям. Полезно для установки связей без лишнего SELECT.

### 3.6. remove()

Удаляет managed-сущность. При flush генерируется DELETE.

**Важные детали:**
- **Сущность должна быть managed**: вызов `remove()` на detached-объекте → `IllegalArgumentException`. Сначала `merge()`.
- **Статус**: `EntityEntry.status = DELETED`. Объект остаётся в Identity Map до закрытия PC.
- **Порядок DELETE**: Hibernate анализирует FK-зависимости и переупорядочивает DELETE-ы: сначала дочерние строки, потом родительские. При нарушении целостности → `ConstraintViolationException`.
- **Каскад**: по умолчанию удаляется только переданная сущность. Для каскада на дочерние: `CascadeType.REMOVE` на родителе. Для удаления «осиротевших» детей при удалении из коллекции: `orphanRemoval = true`.
- **JPQL bulk delete** (`DELETE FROM Entity e WHERE ...`) **не взаимодействует с PC**: не проверяет managed-сущности, не каскадирует, не учитывает `orphanRemoval`. Выполняется напрямую в БД. После bulk delete нужно `clear()` — managed-сущности в PC могут быть уже удалены в БД.
